...
int main(int argc, char *argv[])
{
     ...
     */
    if (argc < 3) {
        fprintf(stderr, "Usage: %s <invert|binarize|noise|smooth|flip|mirrorleft|mirrorright> <PGM image 1> <PGM image 2> ... <PGM image N>\n", argv[0]);
        return 1;
    }
    ...
    for (i = 2; i < argc; ++i) {
	    ...
	    if (strcmp(effect_name, "invert") == 0) {
            ...
            } else if (strcmp(effect_name, "flip") == 0) {
                effect_flip(pgm_info.pixels, pgm_info.width, pgm_info.height);
            } else if (strcmp(effect_name, "mirrorleft") == 0) {
                effect_mirror_left(pgm_info.pixels, pgm_info.width, pgm_info.height);
            } else if (strcmp(effect_name, "mirrorright") == 0) {
                effect_mirror_right(pgm_info.pixels, pgm_info.width, pgm_info.height);
            }
